...
public class BasicShootSO : StrawSO {
    [NamedArray("float", true)] public float[] directions = Array.Empty<float>();
    [NamedArray("float", false)] public float[] directionParameter = Array.Empty<float>();
    ...
    public override IEnumerator ShootDelay(Transform parentBulletTF, float currentTimeValue) {
        for (int j = 0; j < numberWaveShoot; j++) {
            ...
            if (isDelayBetweenWaveShoot) yield return new WaitForSeconds(delayBetweenShoot);
        }
    }
